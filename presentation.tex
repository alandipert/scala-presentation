...
\uncover<1->{class Person(val name:String, val age:Int) \{}
...
\uncover<1->{\}}
\uncover<4->{}
\uncover<4->{val thePrez = new Person("Barack Obama", 48)}
\uncover<5->{}
\uncover<5->{println(thePrez toString)}
\uncover<5->{//prints "My name is Barack Obama and I'm 48 years old."}
\end{semiverbatim}
}
\end{frame} 
...
\uncover<1->{\}}
\uncover<2->{}
\uncover<2->{class Student(val school:String, val name:String, val age:Int) extends Person(name,age) \{}
...
\uncover<1->{\}}
\uncover<1->{}
\uncover<1->{class Student(val school:String, val name:String, val age:Int) extends Person(name,age) \{}
...
\begin{itemize}
  ...
  \item<4-> Prune odd numbers, get sum: \tt{List(1,2,3,4).filter(x => x \% 2 == 0).reduceLeft(\_+\_) (Int = 6)}
\end{itemize}
\end{frame} 
